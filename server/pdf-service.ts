...
async function forceImagesToLoad(page: Page) {
  await page.evaluate(() => {
...
async function waitForFontsOnly(page: Page) {
...
async function prepareBeforeLoad(page: Page) {
...
async function prepareAfterLoad(page: Page) {
...
function getCapturePositions(height: number, maxPages: number) {
...
function normalizeFilename(filename: string, url: URL) {
...
export async function generateWebsitePdf(options: PdfOptions) {
  ...
  const temporaryDirectory = await mkdtemp(
    path.join(tmpdir(), "pdf-generate-rifqi-"),
  );
  ...
  try {
    ...
    const hostnameCache = new Map<string, Promise<void>>();
    ...
    if (response.status() >= 400) {
      throw new Error(
        `Website mengembalikan status HTTP ${response.status()}.`,
      );
    }

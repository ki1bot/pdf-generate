...
async function assertPublicHostname(hostname, cache) {
    ...
    const check = (async () => {
        if (isIP(normalizedHostname)) {
            if (isBlockedAddress(normalizedHostname)) {
                throw new UrlValidationError("Alamat IP privat atau khusus tidak diizinkan.");
            }
            return;
        }
        let addresses;
        try {
            ...
            throw new UrlValidationError("Domain tidak ditemukan atau tidak dapat diakses.");
        }
        if (addresses.length === 0) {
            throw new UrlValidationError("Domain tidak memiliki alamat IP yang valid.");
        }
        if (addresses.some(({ address }) => isBlockedAddress(address))) {
            throw new UrlValidationError("Domain mengarah ke jaringan privat atau khusus.");
        }
    })();
...
export async function validateTargetUrl(rawUrl) {
    ...
    if (url.username || url.password) {
        throw new UrlValidationError("URL dengan username atau password tidak diizinkan.");
    }
    await assertPublicHostname(url.hostname);
    return url;
}
export async function validateResourceUrl(rawUrl, cache) {
